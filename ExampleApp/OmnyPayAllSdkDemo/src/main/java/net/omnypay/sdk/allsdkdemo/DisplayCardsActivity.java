...
import net.omnypay.sdk.allsdkdemo.model.PoSTerminal;
    ...
    private void scanQRCode() {
        try {
            OmnyPayScan.getInstance().start(DisplayCardsActivity.this, new ScannedResultCallback() {
                @Override
                public void onScanResult(final String posString) {
                    ...
                    HttpOps.doGet(merchantPOSId, new Listener() {
                        @Override
                        public void onResult(String response) {
                            Gson gson = new Gson();
                            poSTerminal = gson.fromJson(response, PoSTerminal.class);
                            final String posId = poSTerminal.getId();
                            runOnUiThread(new Runnable() {
                                @Override
                                public void run() {
